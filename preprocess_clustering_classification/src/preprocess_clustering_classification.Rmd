...
# remove urls
annotated_corpus_trans <- tm_map(annotated_corpus_trans, content_transformer(removeURLs))
# remove mentions
annotated_corpus_trans <- tm_map(annotated_corpus_trans, content_transformer(removeMentions))
# remove hastags
annotated_corpus_trans <- tm_map(annotated_corpus_trans, content_transformer(removeHashtags))
# remove newlines
annotated_corpus_trans <- tm_map(annotated_corpus_trans, content_transformer(removeNewlines))
# remove ampersand
annotated_corpus_trans <- tm_map(annotated_corpus_trans, content_transformer(removeAmpersand))
...
findAssocs(annotated_corpus_dtm, term = "young", corlimit = 0.7)
```
## Remove sparse terms
...
wordFreqs <- sort(colSums(as.matrix(annotated_corpus_dtm_99)[1:n, ]), decreasing = TRUE)
wordcloud(words = names(wordFreqs), freq = wordFreqs, max.words = 200, random.order = FALSE, colors = brewer.pal(8, "Dark2"))

wordFreqs <- sort(colSums(as.matrix(annotated_corpus_dtm_99)[(n+1):(n+n), ]), decreasing = TRUE)
wordcloud(words = names(wordFreqs), freq = wordFreqs, max.words = 200, random.order = FALSE, colors = brewer.pal(8, "Dark2"))
```
...
```{r}
distMatrix <- dist(t(scale(as.matrix(annotated_corpus_dtm_95))))
...
```{r}
distMatrix <- dist(scale(as.matrix(annotated_corpus_dtm_95)))
...
inTrain <- createDataPartition(y = annotated_corpus_dtm_99_df$type, p = .75, list = FALSE)
str(inTrain)
training <- annotated_corpus_dtm_99_df[inTrain, ]
testing <- annotated_corpus_dtm_99_df[-inTrain, ]
nrow(training)
```
...
```{r}
knnModelClasses <- predict(knnModel3x10cv, newdata = testing, type = "raw")
confusionMatrix(data = knnModelClasses, testing$type)
```
