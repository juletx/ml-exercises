...
dim(clashroyale_tweets)[1]
clashroyale_tweets$text[1:5]
dim(clashofclans_tweets)[1]
clashofclans_tweets$text[1:5]
```

```{r}
# combine both frames in a single, binary, annotated set
annotated_tweets <- rbind(clashroyale_tweets, clashofclans_tweets)
# interpreting each element of the annotated vector as a document
clashroyale_documents <- VectorSource(clashroyale_tweets$text)
clashofclans_documents <- VectorSource(clashofclans_tweets$text)
# convert to a corpus: supervised classification to be applied in future steps
clashroyale_corpus <- VCorpus(clashroyale_documents)
clashofclans_corpus <- VCorpus(clashofclans_documents)
# merge, concatenate both groups-corpuses
annotated_corpus <- c(clashroyale_corpus, clashofclans_corpus)
```
...
```{r}
# remove numbers
annotated_corpus_trans <- tm_map(annotated_corpus, removeNumbers)
# remove punctuation
annotated_corpus_trans <- tm_map(annotated_corpus_trans, removePunctuation)
# remove urls
annotated_corpus_trans <- tm_map(annotated_corpus_trans, content_transformer(remove_urls))
# remove mentions
annotated_corpus_trans <- tm_map(annotated_corpus_trans, content_transformer(remove_mentions))
# remove hastags
annotated_corpus_trans <- tm_map(annotated_corpus_trans, content_transformer(remove_hashtags))
# remove newlines
annotated_corpus_trans <- tm_map(annotated_corpus_trans, content_transformer(remove_newlines))
# remove emojis
annotated_corpus_trans <- tm_map(annotated_corpus_trans, content_transformer(remove_emojis))
# convert to lowercase
annotated_corpus_trans <- tm_map(annotated_corpus_trans, content_transformer(tolower))
# remove english stop words
annotated_corpus_trans <- tm_map(annotated_corpus_trans, removeWords, stopwords("english"))
# strip whitespace
annotated_corpus_trans <- tm_map(annotated_corpus_trans, stripWhitespace)
# to access Porter's word stemming algorithm
library(SnowballC)
annotated_corpus_trans <- tm_map(annotated_corpus_trans, stemDocument)
```

```{r}
for (i in 1:5) {
    print(annotated_corpus_trans[[i]]$content)
}
for (i in (n + 1):(n + 6)) {
    print(annotated_corpus_trans[[i]]$content)
}
...
```{r}
annotated_corpus_dtm <- DocumentTermMatrix(annotated_corpus_trans)
dim(annotated_corpus_dtm)
findFreqTerms(annotated_corpus_dtm, 15)
findAssocs(annotated_corpus_dtm, term = "clan", corlimit = 0.7)
```
## Remove sparse terms
...
```{r}
annotated_corpus_dtm_95 <- removeSparseTerms(annotated_corpus_dtm, sparse = 0.95)
annotated_corpus_dtm_95
barplot(as.matrix(annotated_corpus_dtm_95),
    xlab = "terms", ylab = "number of occurrences",
    main = "Most frequent terms (sparseness=0.95)"
)
annotated_corpus_dtm_99 <- removeSparseTerms(annotated_corpus_dtm, sparse = 0.99)
annotated_corpus_dtm_99
terms <- dim(annotated_corpus_dtm_99)[2]
barplot(as.matrix(annotated_corpus_dtm_99),
    xlab = "terms", ylab = "number of occurrences",
...
```{r}
ts_plot(dplyr::group_by(annotated_tweets, screen_name), "month") +
...
```{r}
# Remove retweets
clashroyale_organic <- clashroyale_tweets[clashroyale_tweets$is_retweet == FALSE, ]
# Remove replies
clashroyale_organic <- subset(clashroyale_organic, is.na(clashroyale_organic$reply_to_status_id))
# Remove quotes
clashroyale_organic <- clashroyale_organic[clashroyale_organic$is_quote == FALSE, ]
# Keeping only the retweets
clashroyale_retweets <- clashroyale_tweets[clashroyale_tweets$is_retweet == TRUE, ]
# Keeping only the replies
clashroyale_replies <- subset(clashroyale_tweets, !is.na(clashroyale_tweets$reply_to_status_id))
# Keeping only the quotes
clashroyale_quotes <- clashroyale_tweets[clashroyale_tweets$is_quote == TRUE, ]
```

```{r}
# Creating a data frame
clashroyale_data <- data.frame(
    category = c("Organic", "Retweets", "Replies", "Quotes"),
    count = c(dim(clashroyale_organic)[1], dim(clashroyale_retweets)[1], dim(clashroyale_replies)[1], dim(clashroyale_quotes)[1])
)

# Adding columns
clashroyale_data$fraction <- clashroyale_data$count / sum(clashroyale_data$count)
clashroyale_data$percentage <- clashroyale_data$count / sum(clashroyale_data$count) * 100
clashroyale_data$ymax <- cumsum(clashroyale_data$fraction)
clashroyale_data$ymin <- c(0, head(clashroyale_data$ymax, n = -1))

# Rounding the data to two decimal points
clashroyale_data[, -1] <- round(clashroyale_data[, -1], 2)
```
...
word_freqs <- sort(colSums(as.matrix(annotated_corpus_dtm_99)[1:n, ]), decreasing = TRUE)
wordcloud(words = names(word_freqs), freq = word_freqs, max.words = 200, random.order = FALSE, colors = brewer.pal(8, "Dark2"))

word_freqs <- sort(colSums(as.matrix(annotated_corpus_dtm_99)[(n + 1):(n + n), ]), decreasing = TRUE)
wordcloud(words = names(word_freqs), freq = word_freqs, max.words = 200, random.order = FALSE, colors = brewer.pal(8, "Dark2"))
```
...
wordcloud(clashroyale_tweets$hashtags, min.freq = 1, scale = c(3.5, .5), random.order = FALSE, rot.per = 0.35, colors = brewer.pal(8, "Dark2"))
...
wordcloud(clashofclans_tweets$hashtags, min.freq = 1, scale = c(3.5, .5), random.order = FALSE, rot.per = 0.35, colors = brewer.pal(8, "Dark2"))
```
...
annotated_corpus_dtm_99_df <- as.data.frame(as.matrix(annotated_corpus_dtm_99))

# Learn the IsolationForest for our data
iso$fit(annotated_corpus_dtm_99_df)

# predict for our data
p <- iso$predict(annotated_corpus_dtm_99_df)
...
outlierness <- LOF(dataset = annotated_corpus_dtm_99_df, k = 5)

# assign an index to outlierness values
names(outlierness) <- seq_len(nrow(annotated_corpus_dtm_99_df))
...
```{r}
dist_matrix <- dist(scale(as.matrix(annotated_corpus_dtm_95)))
...
```{r}
dim(annotated_corpus_dtm_99)
type <- c(rep("clashroyale", n), rep("clashofclans", n)) # create the type vector
annotated_corpus_dtm_99 <- cbind(annotated_corpus_dtm_99, type) # append
dim(annotated_corpus_dtm_99) # consult the updated number of columns
```
...
```{r}
annotated_corpus_dtm_99_df <- as.data.frame(as.matrix(annotated_corpus_dtm_99))
colnames(annotated_corpus_dtm_99_df)[terms + 1] <- "type"
annotated_corpus_dtm_99_df$type <- as.factor(annotated_corpus_dtm_99_df$type)
annotated_corpus_dtm_99_df <- as.data.frame(sapply(annotated_corpus_dtm_99_df, as.numeric))
annotated_corpus_dtm_99_df[is.na(annotated_corpus_dtm_99_df)] <- 0
annotated_corpus_dtm_99_df$type <- as.factor(annotated_corpus_dtm_99_df$type)
```
...
in_train <- createDataPartition(y = annotated_corpus_dtm_99_df$type, p = .75, list = FALSE)
str(in_train)
training <- annotated_corpus_dtm_99_df[in_train, ]
testing <- annotated_corpus_dtm_99_df[-in_train, ]
nrow(training)
```
...
svm_3x10cv <- train(type ~ ., data = training, method = "svmLinear", trControl = ctrl)
svm_3x10cv
knn_3x10cv <- train(type ~ ., data = training, method = "knn", trControl = ctrl)
knn_3x10cv
```
...
knn_3x10cvacc <- train(type ~ .,
    data = training, method = "knn", trControl = ctrl, tuneLength = 10
)
knn_3x10cvacc
plot(knn_3x10cvacc)
```
...
```{r}
svm_classes <- predict(svm_3x10cv, newdata = testing, type = "raw")
confusionMatrix(data = svm_classes, testing$type)
```

```{r}
knn_classes <- predict(knn_3x10cv, newdata = testing, type = "raw")
confusionMatrix(data = knn_classes, testing$type)
```
...
```{r}
resamps <- resamples(list(knn = knn_3x10cv, svm = svm_3x10cv))
summary(resamps)
...
svm_3x10cv_sbf <- sbf(type ~ ., data = training, method = "svmLinear", trControl = train_ctrl, sbfControl = sbf_ctrl)
svm_3x10cv_sbf
knn_3x10cv_sbf <- sbf(type ~ ., data = training, method = "knn", trControl = train_ctrl, sbfControl = sbf_ctrl)
knn_3x10cv_sbf
```
...
```{r}
rfe_ctrl <- rfeControl(functions = caretFuncs, method = "repeatedcv", repeats = 3)
train_ctrl <- trainControl(method = "repeatedcv", repeats = 3)
svm_3x10cv_rfe <- rfe(type ~ ., data = training, method = "svmLinear", trControl = train_ctrl, rfeControl = rfe_ctrl)
svm_3x10cv_rfe
knn_3x10cv_rfe <- rfe(type ~ ., data = training, method = "knn", trControl = train_ctrl, rfeControl = rfe_ctrl)
knn_3x10cv_rfe
```
...
train_ctrl <- trainControl(method = "repeatedcv", repeats = 3)
svm_3x10cv_gafs <- gafs(x = training[, -ncol(training)], y = training$type, method = "svmLinear", trControl = train_ctrl, gafsControl = gafs_ctrl)
svm_3x10cv_gafs
knn_3x10cv_gafs <- gafs(x = training[, -ncol(training)], y = training$type, method = "knn", trControl = train_ctrl, gafsControl = gafs_ctrl)
knn_3x10cv_gafs
```
...
train_ctrl <- trainControl(method = "repeatedcv", repeats = 3)
svm_3x10cv_safs <- safs(x = training[, -ncol(training)], y = training$type, method = "svmLinear", trControl = train_ctrl, safsControl = safs_ctrl)
svm_3x10cv_safs
knn_3x10cv_safs <- safs(x = training[, -ncol(training)], y = training$type, method = "knn", trControl = train_ctrl, safsControl = safs_ctrl)
knn_3x10cv_safs
```
...
```{r}
pca_res <- prcomp(training[, -ncol(training)])
